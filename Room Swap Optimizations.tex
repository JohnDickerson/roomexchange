...
BiFunction Interface represents a function that accepts two arguments and produces a result. In our case the two arguments are student responses, and the result will be a numerical value-- specifically, an integer. 

\paragraph{Examples}
For example, consider our age response again. The weight function for students' ages would be implemented as follows.

\lstinputlisting[language=Java, caption={Lambda Function}]{lambda_example.java}
...
\lstinputlisting[language=Java, caption={Lambda Function for Location}]{location_example.java}
%\end{multicols}
